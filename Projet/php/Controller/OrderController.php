...
class Order
{
    ...
    public function postGame($param, $jsonData, $game)
    {
        ...
        $idUser = $param->{'x'}->{'user'}->id;
        $game->posteGameWithIdUser(json_encode($gameSave), $nameSave, $idUser, $scoreSave);
        return $jsonData;
    }

    public function getAllGame($param, $jsonData, $game)
    {
        $idUser = $param->{'x'}->{'idUser'};
        $jsonData->setData($game->getAllGameWithIdUser($idUser));
        return $jsonData;
    }
}
